...
function newslite_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && newslite_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Категории : %1$s', 'newslite' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( esc_html( '') );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Метки : %1$s', 'newslite' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
